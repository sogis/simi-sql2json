package ch.so.agi.sql2json;

import ch.so.agi.sql2json.exception.TrafoException;
import ch.so.agi.sql2json.generator.OutputFormat;
import ch.so.agi.sql2json.generator.TextOutput;
import ch.so.agi.sql2json.routing.TemplateWalker;
import ch.so.agi.sql2json.validation.Validator;
import org.slf4j.LoggerFactory;
import org.slf4j.Logger;

import java.io.*;
import java.util.*;

public class Application {

    private static Logger log = LoggerFactory.getLogger(Application.class);

    public static void main(String[] args) throws Exception {

        try {
            Configuration.createConfig4Args(args);

            if(Configuration.helpPrinted())
                return;

            Configuration.assertComplete();

            TextFileReader templateReader = TextFileReader.create(Configuration.valueForKey(Configuration.TEMPLATE_PATH));
            String template = templateReader.readContentToString();

            String outPutPath = Configuration.valueForKey(Configuration.OUTPUT_PATH);
            Hashtable<OutputFormat, TextOutput> output = deferMainOutputFromPath(outPutPath);

            String schemaPath = Configuration.valueForKey(Configuration.JSON_SCHEMA);
            boolean schemaValidation = (schemaPath != null && schemaPath.length() > 0);

            if(schemaValidation && !output.containsKey(OutputFormat.JSON)) { // Falls validiert wird, aber nicht sowieso json erzeugt wird --> Json zu output dazunehmen
                output.put(
                        OutputFormat.JSON,
                        new TextOutput(OutputFormat.JSON, new File(replaceSuffix(outPutPath, "json")))
                );
            }

            TemplateWalker.walkTemplate(template, output.values());
            log.info("Output json written to {}", outPutPath);

            if(schemaValidation){
                log.info("Validating against schema {}", schemaPath);
                String jsonPath = output.get(OutputFormat.JSON).getTextFile().getPath();
                Validator.validate(TextFileReader.create(jsonPath), TextFileReader.create(schemaPath));
            }
        }
        catch (Exception e){
            log.error("Exception occured. Exiting...\n");
            log.error(e.toString());
            e.printStackTrace();

            throw e;
        }
    }

    private static String replaceSuffix(String candidate, String newSuffix){
        String[] parts = candidate.split("\\.");

        if(parts.length < 2)
            throw new TrafoException("Path to data file is malformed. Is {0} but must end with file suffix. Valid examples: '/fuu/data.json', '/bar/data.yaml'", candidate);

        parts[parts.length-1] = newSuffix;

        return String.join(".", parts);
    }

    private static Hashtable<OutputFormat, TextOutput> deferMainOutputFromPath(String outPutPath){

        Hashtable<OutputFormat, TextOutput> res = new Hashtable<>();

        if(outPutPath.endsWith(".json")){
            res.put(
                    OutputFormat.JSON,
                    new TextOutput(OutputFormat.JSON, new File(outPutPath))
            );
        }
        else if(outPutPath.endsWith(".yaml")){
            res.put(
                    OutputFormat.YAML,
                    new TextOutput(OutputFormat.YAML, new File(outPutPath))
            );
        }
        else{
            throw new TrafoException("Failed to defer json path from text file output path. File name in output path must end with either '.json' or '.yaml'.");
        }

        return res;
    }
}
